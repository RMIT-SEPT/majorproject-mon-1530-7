...
class CustomerPastBookingsPage extends Component {
  ...
  renderAppropriateCustomerPastBookingComponent() {
    if (
      this.props.loadingCustomerPastBookingsPage &&
      !this.props.loadingCustomerDashboardBookingHistorySection
    ) {
  ...
  render() {
    return <div>{this.renderAppropriateCustomerPastBookingComponent()}</div>;
  }
}
